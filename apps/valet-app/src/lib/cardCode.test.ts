...
describe('parseCardCode', () => {
  it('reads the code out of a card QR', () => {
    expect(parseCardCode('https://dwaarai.com/valet/c/A047')).toBe('A047');
  });

  it('handles a trailing slash and a query string', () => {
    expect(parseCardCode('https://dwaarai.com/valet/c/A047?utm=x')).toBe('A047');
    expect(parseCardCode('https://dwaarai.com/valet/c/A047#top')).toBe('A047');
  });
  ...
  it('refuses a code too long for the column', () => {
    ...
    expect(parseCardCode(`https://dwaarai.com/valet/c/${'X'.repeat(21)}`)).toBeNull();
  });
  ...
  it('decodes a percent-encoded segment', () => {
    expect(parseCardCode('https://dwaarai.com/valet/c/A%2D047')).toBe('A-047');
  });
});
