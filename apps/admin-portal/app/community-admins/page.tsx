  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Community Admins</h1>
        <button
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Username</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400">No community admins yet</td></tr>
            ) : (
              admins.map((a) => (
                <tr key={a.id} className="border-b border-gray-200/50 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 text-sm text-gray-700 font-medium">{a.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-500 font-mono">{a.username}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{a.community_name || '—'}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center">
          <div className="glass-panel gradient-border p-6 w-full max-w-md">
            <h2 className="text-xl font-bold text-gray-900 mb-4">New Community Admin</h2>
            <div className="space-y-3">
              ...
              >
                <option value="" className="bg-white">Select community *</option>
                {communities.map((c) => (
